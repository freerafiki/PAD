...
class MultiModalScorerV2(nn.Module):
    ...
    def __init__(self, 
        ...
        print(f"DINO model loaded (frozen). Output dimension: {self.dino.config.hidden_size}")
        ...
        if use_cross_attention:
            self.geom_to_visual = CrossModalAttention(dim=768, num_heads=8, dropout=dropout)
            self.visual_to_geom = CrossModalAttention(dim=768, num_heads=8, dropout=dropout)
    ...
    def forward(self, rgb, rgb_geometric):
        ...
        if self.use_cross_attention:
            ...
            geom_attended = self.geom_to_visual(geom_feats_seq, dino_feats_seq).squeeze(1)
            visual_attended = self.visual_to_geom(dino_feats_seq, geom_feats_seq).squeeze(1)
